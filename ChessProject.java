...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
  ...
  private static int strategyChoice;
  ...
  private Stack getWhitePawnSquares(int x, int y, String piece) {
    ...
    //Validanting the pawn from first position 1 or 2 moves
    for (int i = 1; i <= 2; i++) {
      int tmpx = x;
      //move up 1 or 2 squares
      int tmpy = y + i;
    ...
    // Validating the diagonal attacking move ro right
    for (int i = 1; i < 2; i++) {
      int tmpx = x + i;
      int tmpy = y + i;
      if (!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)) {
    ...
    // Validating the diagonal attacking move to left
    for (int i = 1; i < 2; i++) {
      int tmpx = x - i;
      int tmpy = y + i;
      if (!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)) {
  ...
  private void colorBlackSquares(Stack squares) {
    Border greenBorder = BorderFactory.createLineBorder(Color.YELLOW, 3);
    while (!squares.empty()) {
      Square s = (Square) squares.pop();
      int location = s.getXC() + ((s.getYC()) * 8);
      JPanel panel = (JPanel) chessBoard.getComponent(location);
  ...
  private void getBlackLandingSquares(Stack found) {
    Move tmp;
    Square landing;
    Stack squares = new Stack();
    while (!found.empty()) {
  ...
  // private Stack getWhiteAttackingSquare(Stack pieces) {
  //   while (!pieces.empty()) {
  ...
  //         System.out.println("Piece: " + tempKnight.pieceName + "\nCoordinates: " + tempKnight.getXC() + ";" + tempKnight.getYC());
  //       }
  //     }
  //   }
  // }
  ...
  private void printStack(Stack input) {
    ...
    while (!input.empty()) {
      ...
      System.out.println("Move: " + m.getStart().pieceName + " (" + s.getXC() + " , " + s.getYC() + "), landing at (" +
        l.getXC() + " , " + l.getYC() + ")");
    }
  }
  ...
  private void makeAIMove() {
    ...
    Stack importWhite = findWhitePieces();
    ...
    // System.out.println("+++++++++++++++WHITE PIECES PRINTED+++++++++++++++");
    printStack(temporary);

    Stack blackMoves = (Stack) black.clone();
    while (!blackMoves.empty()) {
      Square s = (Square) blackMoves.pop();
      String tmpString = s.getName();
      Stack tmpMoves = new Stack();
    ...
    getBlackLandingSquares(blackTemporary);
    // System.out.println("+++++++++++++++BLACK PIECES AVAILABLE MOVES+++++++++++++++");
    ...
    if (completeMoves.size() == 0) {
      ...
      System.out.println("+++++++++++++++WHITE PIECES AVAILABLE MOVES+++++++++++++++");
      while (!completeMoves.empty()) {
        ...
        System.out.println("Move " + s1.getName() + ": start (" + s1.getXC() + ", " + s1.getYC() +
          ") landing: (" + s2.getXC() + ", " + s2.getYC() + ")");
        possibleMoves.push(tmpMove);
      }
      System.out.println("=============================================================");
      Border redBorder = BorderFactory.createLineBorder(Color.RED, 3);
      //Initializing the selectedMove
      Move selectedMove;
      //Return the choice of the strategy picked by the user
      switch (strategyChoice) {
        ...
        default: //code will not compile without this, game needs to execute one of the AI AGENTSs
          selectedMove = agent.twoLevelsDeep(importWhite, possibleMoves, black, blackCompleteMoves);
          break;
      }
